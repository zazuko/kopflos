...
class SparqlView extends ApiObject {
  constructor (options) {
    ...
    this.variablesIri = this.valueTerm(ns.hydraView.variables)
    ...
    const returnCsvMetadataIri = this.valueTerm(ns.hydraView.returnCsvMetadata)

    if (returnCsvMetadataIri) {
      const metadata = JSON.parse(fs.readFileSync(path.resolve(options.basePath, returnCsvMetadataIri.value.slice(7))).toString())
