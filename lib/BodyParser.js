...
class BodyParser extends ApiObject {
  ...
  initJsonld () {
    this.returnFrame = this.valueTerm(ns.hydraView.returnFrame)
  ...
  initCsv () {
    this.returnCsvMetadata = this.valueTerm(ns.hydraView.returnCsvMetadata)
  ...
  _handle (req, res, next) {
    ...
    rdfBodyParser.attach(req, res, {formats: this.customFormats}).then(next).catch(next)
  }
}
