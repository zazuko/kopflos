...
function findOperation (api, operationMiddleware) {
  return async function (req, res, next) {
    const method = req.method === 'HEAD' ? 'GET' : req.method

    let operations = mapOperations({ api, req, res, method })
      .filter(({ operation }) => operation)
...
function factory (api, middleware = {}) {
  const router = Router()

  router.use(findOperation(api, middleware.operations))
